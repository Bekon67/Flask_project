...
def add_words(cur, new):
    ...
    # print(res)
    if res:
        # print(res[2], new['count'])
        if res[2] < new['count']:
            # обновление строки таблицы
            cur.execute('update words set count = ?, salary_down = ?, salary_up = ? where words.id = ?',
        ...
        cur.execute('insert into words values (null, ?, ?, ?, ?)',
                    (new['keywords'], new['count'], new['salary_down'], new['salary_up']))
        print('Done')
    return cur
...
def add_ws(cur, new):
    ...
    for item in new['requirements']:
        ...
        cur.execute('select * from words_skills as ws where ws.id_word = ? and ws.id_skill = ?',
                    (word_id, skill_id))
        res = cur.fetchone()
        if not res:
            cur.execute('insert into words_skills values (null, ?, ?, ?, ?)',
                        (word_id, skill_id, item['count'], item['percent']))
            print('ws done')
        elif word_count < new['count']:
            cur.execute('update words_skills as ws set count = ?, percent = ? where ws.id_word = ? and ws.id_skill = ?',
